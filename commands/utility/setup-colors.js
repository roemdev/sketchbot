...
function buildColorsPanel() {
  ...
  // Máximo 5 botones por fila — dividimos en chunks
...
module.exports = {
  ...
  async execute(interaction) {
    await interaction.channel.send({
      components: [buildColorsPanel()],
      flags: MessageFlags.IsComponentsV2,
    });

    return interaction.reply({
      content: "Panel de colores enviado correctamente.",
  ...
  async buttonHandler(interaction) {
    ...
    // Quitar todos los roles de color que el usuario ya tenga
    const toRemove = [...member.roles.cache.keys()].filter((id) =>
      ALL_COLOR_IDS.has(id)
    );
    ...
    // Si ya tenía ese rol, lo quitamos (toggle off). Si no, lo añadimos.
    if (!hadRole) {
      await member.roles.add(roleId);
      const role = COLOR_ROLES.find((r) => r.id === roleId);
      return interaction.editReply({
        content: `${role.emoji} Se te asignó el rol **${role.name}**.`,
      });
    }

    return interaction.editReply({
      content: "Se quitó tu rol de color.",
    });
  },
};
