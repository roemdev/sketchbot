...
function createButtons() {
  return new ActionRowBuilder()
    .addComponents(
      new ButtonBuilder().setCustomId('nb_update').setEmoji('🔃').setStyle(ButtonStyle.Secondary),
...
async function handleButtonInteraction(interaction) {
  ...
  if (interaction.customId === 'nb_update') {
    await interaction.reply({ content: 'Presionaste el botón 🔃nb_update.', flags: MessageFlags.Ephemeral });
  } else if (interaction.customId === 'nb_my_donation') {
