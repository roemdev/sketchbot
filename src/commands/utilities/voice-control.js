...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription(
      "Controla los permisos de un canal de voz con botones interactivos."
    )
  ...
  async execute(interaction) {
    ...
    const filter = (i) =>
      buttons.some((button) => button.data.custom_id === i.customId);
    ...
    collector.on("collect", async (i) => {
      ...
      if (!voiceChannel) {
        const warningEmbed = new EmbedBuilder()
          ...
          .setDescription(
            `${assets.emoji.warn} <@${i.user.id}>: No estás **conectado** a un canal de voz.`
          );
        return i.reply({
          embeds: [warningEmbed],
      ...
      if (!isOwner) {
        const warningEmbed = new EmbedBuilder()
          .setColor(assets.color.yellow)
          .setDescription(
            `${assets.emoji.warn} <@${i.user.id}>: No eres el **propietario** de este canal de voz.`
          );
        return i.reply({
          embeds: [warningEmbed],
          flags: MessageFlags.Ephemeral,
...
async function toggleChannelLock(interaction, channel, state) {
  ...
  const responseEmbed = new EmbedBuilder()
    ...
    .setDescription(
      `${newState ? assets.emoji.warn : assets.emoji.check} <@${
        i.user.id
      }>: Tu canal de voz fue ${
        newState ? "**bloqueado**" : "**desbloqueado**"
      }.`
    );
  ...
  const responseEmbed = new EmbedBuilder()
    ...
    .setDescription(
      `${newState ? assets.emoji.warn : assets.emoji.check} <@${
        ...
        newState ? "se ha **ocultado**" : "ahora es **visible**"
      }.`
    );
  await interaction.reply({
    embeds: [responseEmbed],
...
async function handleMemberKick(interaction, channel) {
  ...
  if (members.length === 0) {
    const responseEmbed = new EmbedBuilder()
      .setColor(assets.color.yellow)
      .setDescription(
        `${assets.emoji.warn} <@${i.user.id}>: No hay miembros para **expulsar**.`
      );
  ...
  collector.on("collect", async (selectInteraction) => {
    ...
    if (!member) {
      const responseEmbed = new EmbedBuilder()
        .setColor(assets.color.yellow)
        .setDescription(
          `${assets.emoji.warn} <@${i.user.id}>: No se pudo encontrar al miembro seleccionado.`
        );
    ...
    if (memberId === interaction.user.id) {
      const responseEmbed = new EmbedBuilder()
        .setColor(assets.color.yellow)
        .setDescription(
          `${assets.emoji.warn} <@${i.user.id}>: No puedes **expulsarte** de tu propio canal.`
        );
      return selectInteraction.reply({
        embeds: [responseEmbed],
        flags: MessageFlags.Ephemeral,
    ...
    const responseEmbed = new EmbedBuilder()
      ...
      .setDescription(
        `${assets.emoji.check} <@${i.user.id}>: El usuario **${member.user.username}** ha sido **expulsado** de tu canal de voz.`
      );
...
async function handleInfoButton(interaction, channel) {
  if (!channel) {
    const responseEmbed = new EmbedBuilder()
      .setColor(assets.color.yellow)
      .setDescription(
        `${assets.emoji.warn} <@${i.user.id}>: No estás en un **canal de voz**.`
      );
    return interaction.reply({
      embeds: [responseEmbed],
      flags: MessageFlags.Ephemeral,
  ...
  const infoEmbed = new EmbedBuilder()
    ...
    .setAuthor({
      name: "Información del canal de voz",
